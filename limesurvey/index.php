...
function buildsurveysession($previewGroup=false)
{
    ...
    if ($tokensexist == 1 && !returnglobal('token') && !$previewGroup)
    {
        ...
        else
        {
            ...
            <li>
            <label for='token'>" .$clang->gT("Can't bind to the LDAP directory")."</label>
            <input class='text $kpclass' id='company' type='text' name='company' />";
    ...
    !captcha_enabled('surveyaccessscreen',$thissurvey['usecaptcha']))
    {
        ...
        if ($thissurvey['alloweditaftercompletion'] == 'Y' ) {
          ...
          //  $tkquery = "SELECT COUNT(*) FROM ".db_table_name('tokens_'.$surveyid)." WHERE token='".db_quote(trim(strip_tags(returnglobal('token'))))."' AND firstname='".db_quote(trim(strip_tags(returnglobal('company'))))."'";
        } else {
       //     $tkquery = "SELECT COUNT(*) FROM ".db_table_name('tokens_'.$surveyid)." WHERE token='".db_quote(trim(strip_tags(returnglobal('token'))))."' AND (completed = 'N' or completed='') AND firstname='".db_quote(trim(strip_tags(returnglobal('company'))))."'";
